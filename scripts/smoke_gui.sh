...
# Starts rbotd in background (if available) and launches the Gio settings UI.
# Usage: ./scripts/smoke_gui.sh

RBOTD_BIN=./bin/rbotd
GUI_BIN=./bin/rbot-settings-gio
...
if [ -x "$GUI_BIN" ]; then
	...
	echo "Launching GUI via 'go run cmd/rbot-settings-gio'"
	(cd "$(git rev-parse --show-toplevel)" && go run cmd/rbot-settings-gio)
fi
